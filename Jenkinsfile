pipeline {
    ...
    stages {
     ...
     stage('Dev-Deploy') {
            ...
            sh "/var/lib/jenkins/service-status.sh `hyscalectl get svc -e dev -a HRMS | awk '/hrms-frontend/ {print ${7}}'`"
        }
    }
     stage('Stage-Deploy') {
            steps {
            ...
            sh "/var/lib/jenkins/service-status.sh `hyscalectl get svc -e stage -a HRMS | awk '/hrms-frontend/ {print ${7}}'`"
        }
    }
}
